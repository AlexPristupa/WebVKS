...
} from '@/pages/booking/BookingsRestricted/modals/config/editBookingPeriodicModal.interface'
...
import { dayStringFormators } from '@/pages/booking/BookingsRestricted/modals/EditBookingPeriodicModal/config/editPeriodicDaily.const'
...
>({
  ...
  mounted() {
    this.entityModel = JSON.parse(JSON.stringify(this.selectedEntity))
    this.setDefaults(this.selectedEntity.schedule)
  },
  methods: {
    setTab(tabNumber) {
    ...
    setDefaults(schedule) {
      if (this.tab === 1) {
        ...
        if (schedule) {
          const quantity = isNaN(Number(schedule.substring(0, 1)))
            ? 1
            : Number(schedule.substring(0, 1))
          if (this.dependentTab === 1) {
            this.initData.quantity1 = quantity
          }
          if (this.dependentTab === 2) {
            this.initData.frequency2 = schedule.includes(
              ...
              ? 1
              : 2
            this.initData.quantity2 = quantity
            return
          }
        }
      }
      ...
      if (this.tab === 3) {
        ...
        if (schedule) {
          const monthDay = Number(schedule.substring(5, 6))
          if (this.dependentTab === 1) {
            this.initData.monthDay1 = isNaN(monthDay) ? 1 : monthDay
            return
          }
          if (this.dependentTab === 2) {
            return
          ...
          if (this.dependentTab === 4) {
            ...
            this.initData.monthDay4 = isNaN(monthDay) ? 1 : monthDay
